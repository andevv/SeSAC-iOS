    ...
    private func bind() {
        ...
        users
            .bind(to: tableView.rx.items) { tableView, row, element in
                guard let cell = tableView.dequeueReusableCell(withIdentifier: PersonTableViewCell.identifier, for: IndexPath(row: row, section: 0)) as? PersonTableViewCell else { return UITableViewCell() }
        ...
        .bind(with: self) { owner, value in
            let person = value.0
            var names = try! owner.selectedNames.value()
            names.append(person.name)
            owner.selectedNames.onNext(names)
        }
        .disposed(by: disposeBag)
        
        // 컬렉션뷰 바인딩
        selectedNames
            .bind(to: collectionView.rx.items) { collectionView, row, name in
                guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: UserCollectionViewCell.identifier, for: IndexPath(row: row, section: 0)) as? UserCollectionViewCell else { return UICollectionViewCell() }
        ...
        searchBar.rx.searchButtonClicked
            .bind(with: self) { owner, _ in
                guard let text = owner.searchBar.text?
                        .trimmingCharacters(in: .whitespacesAndNewlines),
                      !text.isEmpty else { return }
                
                var current = try! owner.users.value()
                current.append(
                    Person(name: text,
                           email: "\(text.lowercased())@example.com",
                           profileImage: "dummy")
                )
                owner.users.onNext(current)
